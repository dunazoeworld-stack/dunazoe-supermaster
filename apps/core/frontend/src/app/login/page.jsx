...
export default function LoginPage() {
  ...
  async function handleSubmit(e){
    ...
    try{
      ...
      if(!data.success){ setError(data.error||(data.locked_for_minutes?`Account locked for ${data.locked_for_minutes} minutes`:"Invalid credentials")); return; }
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0A0E1A",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      <div style={{width:"100%",maxWidth:"420px",background:"linear-gradient(145deg,#0D1525,#0A1020)",border:"1px solid rgba(0,163,255,0.2)",borderRadius:"24px",padding:"40px",boxShadow:"0 24px 80px rgba(0,0,0,0.5)"}}>
        ...
        <div style={{marginTop:"24px",textAlign:"center",fontSize:"0.85rem",color:"#8A9AB5"}}>Don't have an account? <Link href="/register" style={{color:"#00A3FF",textDecoration:"none",fontWeight:600}}>Sign up free</Link></div>
      </div>
    </div>
  );
}
