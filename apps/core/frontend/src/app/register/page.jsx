    ...
    <div style={{ minHeight: "100vh", background: "#0A0E1A", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      <div style={{ width: "100%", maxWidth: "460px", background: "linear-gradient(145deg,#0D1525,#0A1020)", border: "1px solid rgba(0,163,255,0.2)", borderRadius: "24px", padding: "40px", boxShadow: "0 24px 80px rgba(0,0,0,0.5)" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div>
            <label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Full Name</label>
            <input type="text" autoComplete="name" value={form.name} onChange={set("name")} placeholder="Temidayo Folorunso" required style={inp} />
          </div>
          <div>
            <label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Email</label>
            <input type="email" autoComplete="email" value={form.email} onChange={set("email")} placeholder="you@example.com" required style={inp} />
          </div>
          <div>
            <label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Phone (WhatsApp)</label>
            <input type="tel" autoComplete="tel" value={form.phone} onChange={set("phone")} placeholder="+2348012345678" required style={inp} />
          </div>
          {role === "vendor" && (
            <div>
              <label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Business / Store Name</label>
              <input type="text" autoComplete="organization" value={form.business_name} onChange={set("business_name")} placeholder="My Fashion Store" required style={inp} />
            </div>
          )}
          <PwdField label="Password" field="password" show={showPwd} toggle={() => setShowPwd(v => !v)} placeholder="Min 8 characters" autoComp="new-password" />
          <PwdField label="Confirm Password" field="confirm" show={showConfirm} toggle={() => setShowConfirm(v => !v)} placeholder="Repeat password" autoComp="new-password" />

          <button type="submit" disabled={loading}
            style={{ padding: "14px", borderRadius: "12px", background: loading ? "rgba(0,163,255,0.4)" : "linear-gradient(135deg,#00A3FF,#0066FF)", border: "none", color: "#fff", fontSize: "1rem", fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", marginTop: "4px" }}>
            {loading ? "Creating account…" : `Create ${role === "vendor" ? "Vendor " : ""}Account →`}
          </button>
        </form>

        <p style={{ marginTop: "12px", fontSize: "0.75rem", color: "#3D4F6E", textAlign: "center", lineHeight: 1.5 }}>
          By registering you agree to DUNAZOE's Terms of Service and Privacy Policy.
        </p>
        <div style={{ marginTop: "20px", textAlign: "center", fontSize: "0.85rem", color: "#8A9AB5" }}>
          Already have an account? <Link href="/login" style={{ color: "#00A3FF", textDecoration: "none", fontWeight: 600 }}>Sign in</Link>
        </div>
      </div>
    </div>
  );
