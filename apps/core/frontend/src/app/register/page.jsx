    ...
    <div style={{ minHeight: "100vh", background: "#0A0E1A", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      <div style={{ width: "100%", maxWidth: "460px", background: "linear-gradient(145deg,#0D1525,#0A1020)", border: "1px solid rgba(0,163,255,0.2)", borderRadius: "24px", padding: "40px", boxShadow: "0 24px 80px rgba(0,0,0,0.5)" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Full Name</label><input type="text" autoComplete="name" value={form.name} onChange={set("name")} placeholder="Temidayo Folorunso" required style={inp} /></div>
          <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Email</label><input type="email" autoComplete="email" value={form.email} onChange={set("email")} placeholder="you@example.com" required style={inp} /></div>
          <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Phone (WhatsApp)</label><input type="tel" autoComplete="tel" value={form.phone} onChange={set("phone")} placeholder="+2348012345678" required style={inp} /></div>
          {role === "vendor" && <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Business / Store Name</label><input type="text" autoComplete="organization" value={form.business_name} onChange={set("business_name")} placeholder="My Fashion Store" required style={inp} /></div>}
          <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Password</label><input type="password" autoComplete="new-password" value={form.password} onChange={set("password")} placeholder="Min 8 characters" required style={inp} /></div>
          <div><label style={{ fontSize: "0.82rem", color: "#8A9AB5", marginBottom: "6px", display: "block" }}>Confirm Password</label><input type="password" autoComplete="new-password" value={form.confirm} onChange={set("confirm")} placeholder="Repeat password" required style={inp} /></div>
