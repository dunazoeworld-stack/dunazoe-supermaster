...
export default function ReleasesPage() {
  ...
  async function load(t) {
    setLoading(true);
    try {
      const r = await fetch(`${API}/deployment/releases`, { headers:{ Authorization:`Bearer ${t||token}` } });
    ...
    <div style={{ minHeight:"100vh", background:"#0A0E1A", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px" }}>
      <div style={{ width:"100%", maxWidth:"360px", background:"linear-gradient(145deg,#0D1525,#0A1020)", border:"1px solid rgba(0,163,255,0.2)", borderRadius:"20px", padding:"32px" }}>
        <div style={{ textAlign:"center", marginBottom:"20px" }}>
          ...
          <h1 style={{ marginTop:"10px", fontSize:"1.2rem", fontWeight:800, background:"linear-gradient(135deg,#00A3FF,#0066FF)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>Releases</h1>
        </div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0A0E1A", padding:"16px 14px 80px", maxWidth:"500px", margin:"0 auto" }}>
      <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"18px" }}>
        ...
        <div style={{ flex:1 }}>
          <h1 style={{ fontSize:"1rem", fontWeight:800, margin:0, background:"linear-gradient(135deg,#00A3FF,#0066FF)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>Release History</h1>
          <p style={{ fontSize:"0.65rem", color:"#3D4F6E", margin:0 }}>All tagged versions</p>
        </div>
      ...
      {data && (
        <div style={{ background:"rgba(0,163,255,0.06)", border:"1px solid rgba(0,163,255,0.2)", borderRadius:"12px", padding:"12px 16px", marginBottom:"14px" }}>
          <p style={{ margin:0, fontSize:"0.72rem", color:"#3D4F6E", fontWeight:700 }}>CURRENT VERSION</p>
          <p style={{ margin:0, fontSize:"1.4rem", fontWeight:900, color:"#00A3FF" }}>{data.current_version}</p>
        </div>
      )}
      ...
      {loading && !data && <div style={{ textAlign:"center", padding:"40px", color:"#3D4F6E" }}>Loading releases…</div>}

      <div style={{ background:"rgba(13,21,37,0.9)", border:"1px solid rgba(0,163,255,0.08)", borderRadius:"14px", padding:"14px 16px" }}>
        <p style={{ fontSize:"0.68rem", color:"#3D4F6E", fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", margin:"0 0 10px" }}>All Releases</p>
        {data?.releases?.map((r,i) => (
          <div key={i} style={{ padding:"11px 0", borderBottom:"1px solid rgba(255,255,255,0.04)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              <p style={{ margin:0, fontSize:"0.9rem", fontWeight:800, color: r.passed?"#00CC88":"#FF3B5C" }}>v{r.version}</p>
              <p style={{ margin:0, fontSize:"0.7rem", color:"#3D4F6E" }}>{r.environment} · {r.released_at ? new Date(r.released_at).toLocaleDateString("en-NG") : "—"}</p>
            </div>
            <div style={{ textAlign:"right" }}>
              <p style={{ margin:0, fontSize:"1.1rem", fontWeight:900, color:"#00A3FF" }}>{r.score ?? "—"}<span style={{ fontSize:"0.6rem", color:"#3D4F6E" }}>/100</span></p>
              <p style={{ margin:0, fontSize:"0.68rem", color: r.passed?"#00CC88":"#FF3B5C" }}>{r.passed?"✅ PASSED":"❌ BLOCKED"}</p>
            </div>
          </div>
        ))}
      </div>
