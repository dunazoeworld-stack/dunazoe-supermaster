...
function CopyBtn({ text }) {
  ...
  return <button onClick={copy} style={{ padding:"3px 8px", borderRadius:"6px", background:"rgba(0,163,255,0.1)", border:"1px solid rgba(0,163,255,0.2)", color:"#00A3FF", fontSize:"0.65rem", cursor:"pointer", flexShrink:0 }}>{copied?"✓":"copy"}</button>;
}
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0A0E1A", padding:"16px 14px 80px", maxWidth:"500px", margin:"0 auto" }}>
      <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"18px" }}>
        ...
        <div style={{ flex:1 }}>
          <h1 style={{ fontSize:"1rem", fontWeight:800, margin:0, background:"linear-gradient(135deg,#00A3FF,#0066FF)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>GitHub Push Engine</h1>
          <p style={{ fontSize:"0.65rem", color:"#3D4F6E", margin:0 }}>Release commands — run in Termius</p>
        </div>
      ...
      {data && (
        <>
          <div style={{ background:"rgba(13,21,37,0.9)", border:"1px solid rgba(0,163,255,0.12)", borderRadius:"14px", padding:"14px 16px", marginBottom:"12px" }}>
            <p style={{ fontSize:"0.68rem", color:"#3D4F6E", fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", margin:"0 0 10px" }}>Repository</p>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"6px" }}>
              <span style={{ fontSize:"0.78rem", color:"#8A9AB5" }}>Repo</span>
              <span style={{ fontSize:"0.78rem", color:"#cdd5e0", fontWeight:600 }}>{data.repo}</span>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"6px" }}>
              <span style={{ fontSize:"0.78rem", color:"#8A9AB5" }}>Branch</span>
              <span style={{ fontSize:"0.78rem", color:"#00A3FF", fontWeight:600 }}>{data.branch}</span>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"6px" }}>
              <span style={{ fontSize:"0.78rem", color:"#8A9AB5" }}>Tag</span>
              <span style={{ fontSize:"0.78rem", color:"#00CC88", fontWeight:600 }}>{data.tag}</span>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <span style={{ fontSize:"0.78rem", color:"#8A9AB5" }}>Status</span>
              <span style={{ fontSize:"0.78rem", color:"#00CC88", fontWeight:700 }}>{data.status}</span>
            </div>
          </div>

          <div style={{ background:"rgba(13,21,37,0.9)", border:"1px solid rgba(0,163,255,0.08)", borderRadius:"14px", padding:"14px 16px" }}>
            <p style={{ fontSize:"0.68rem", color:"#3D4F6E", fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", margin:"0 0 10px" }}>Push Commands (run in order)</p>
            {data.push_commands?.map((cmd, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"8px 10px", background:"rgba(0,0,0,0.3)", borderRadius:"8px", marginBottom:"6px", fontFamily:"monospace" }}>
                <span style={{ fontSize:"0.68rem", color:"#3D4F6E", minWidth:"16px" }}>{i+1}.</span>
                <span style={{ fontSize:"0.75rem", color:"#00CC88", flex:1, wordBreak:"break-all" }}>{cmd}</span>
                <CopyBtn text={cmd} />
              </div>
            ))}
          </div>

          <div style={{ background:"rgba(245,166,35,0.06)", border:"1px solid rgba(245,166,35,0.2)", borderRadius:"10px", padding:"12px", marginTop:"12px" }}>
            <p style={{ margin:0, fontSize:"0.78rem", color:"#F5A623", fontWeight:600 }}>⚠️ Do NOT push automatically. Require approval. Use Termius on phone.</p>
          </div>
        </>
      )}
