            ...
            <div style={{ paddingTop: "20px", fontSize: "0.82rem", color: "var(--text-muted)" }}>
              {breadcrumb.map((b, i) => (
                <span key={i}>
                  ...
                  {b.href ? <Link href={b.href} style={{ color: "var(--dz-blue)", textDecoration: "none" }}>{b.label}</Link> : <span>{b.label}</span>}
                </span>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
              {[
                ...
                <Link key={href} href={href} style={{ fontSize: "0.82rem", color: "var(--text-muted)", textDecoration: "none" }}>{label}</Link>
              ))}
            </div>
